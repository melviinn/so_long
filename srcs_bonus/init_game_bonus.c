...
void	init_game(t_game *g)
{
	...
	g->win = mlx_new_window(g->mlx, g->map.columns * IMG_WIDTH, g->map.rows
			* IMG_HEIGHT, "so_long");
	if (!g->win)
	{
		free(g->win);
...
void	init_sprites(t_game *g)
{
	...
	g->coins = new_sprite(mlx, "textures/bonus/coin_0.xpm", g);
	g->enemy = new_sprite(mlx, "textures/bonus/enemy.xpm", g);
...
void	start_gameplay(t_game *g)
{
	mlx_hook(g->win, KeyPress, KeyPressMask, handle_input, g);
	mlx_hook(g->win, EVENT_CLOSE_BTN, 0, close_handler, g);
	mlx_loop_hook(g->mlx, refresh_game, g);
}
