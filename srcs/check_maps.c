...
static void	check_rows(t_game *g)
{
	...
	while (i < g->map.rows)
	{
		if (g->map.map[i][0] != WALL)
			error_msg("Invalid Map: missing wall in the first column.", g);
		else if (g->map.map[i][g->map.columns - 1] != WALL)
			error_msg("Invalid Map: missing wall in the last column.", g);
		i++;
	}
}
...
static void	check_columns(t_game *g)
{
	...
	while (i < g->map.columns)
	{
		if (g->map.map[0][i] != WALL)
			error_msg("Invalid Map: missing wall in the first row.", g);
		else if (g->map.map[g->map.rows - 1][i] != WALL)
			error_msg("Invalid Map: missing wall in the last row.", g);
		i++;
	}
}
...
static void	count_map_params(t_game *g)
{
	...
	while (y < g->map.rows)
	{
		...
		{
			if (!ft_strchr_new("CEP01", g->map.map[y][x]))
